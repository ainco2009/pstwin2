...
IF lEncripta                && Se ha llamado a la función para encriptar
	...
	IF !EMPTY(cCadena)
		...
		cCadenaTemporal = PADR(cCadena,nLongitud)
...
*-------------------------------------------------------
* Función.....: ImpPdf(lcnomfrx,lcarchivopdf, lcDirPDF, lcFiltro, llCombina)

*
* Descripción.: Improme un reporte en formato PDF utilizando PDFcreator.
*
* Parámetros..: lcnomfrx - Ruta y nombre del reporte
*               lcarchivopdf  - Nombre del archivi PDF a generar
*               lcDirPDF    - Ruta del archivi PDF
*				lcFiltro -    Filtro para usar en la clausula FOR del reporte
*
*
* Devuelve....: logico
*
* Notas.......: 03/07/2010 10:38:28 Pedro Amaro
*-------------------------------------------------------

FUNCTION ImpPdf2
PARAMETERS lcnomfrx,lcarchivopdf, lcDirPDF, lcFiltro, llCombina
LOCAL mnom, llok, DefaultPrinter, c, bFiltro, lcarchivoPs, lcErr, loError
llok = .T.

IF PARAMETERS() < 4
	lcFiltro= ""
ENDIF
bFiltro = IIF(EMPTY(lcFiltro),"",lcFiltro)
...
TRY
	lcErr = ""
	IF EMPTY(lcarchivopdf)
		mnom='PDFCreator'
		SET PRINTER TO NAME (mnom)
		REPORT FORM (lcnomfrx) TO PRINTER NOCONSOLE
		SET PRINTER TO DEFAULT

	ELSE
		IF !DIRECTORY(lcDirPDF)
			MKDIR(lcDirPDF)
		ENDIF
		...
		IF TYPE('PDFCreator') # 'O'
			lcErr ="No se puede procesar el archivo porque no tiene la aplicación" ;
				+ CHR(13) + "PDFCreator instalada en su computador."
			EXIT
		ENDIF
		PDFReady = CREATEOBJECT("PDFEvent") && Ver definición de la clase más bajo
		IF TYPE('PDFReady') # 'O'
			lcErr = "No se puede crear la clase PDFready"
			EXIT
		ENDIF
		...
		WITH PDFCreator
* Arrancar sin lanzar los trabajos
			.cStart ("/NoProcessingAtStartup")
* Options de autoguardado :
			.cOption("UseAutosave") = 1
			.cOption("UseAutosaveDirectory") = 1
			.cOption("AutosaveDirectory") = lcDirPDF
			.cOption("AutosaveFilename") = lcarchivopdf
...
* Modificacion temporal de lo impresora por defecto :
		...
		IF EMPTY(bFiltro)
			REPORT FORM (lcnomfrx)  TO PRINTER  NOCONSOLE NODIALOG
		ELSE
			REPORT FORM (lcnomfrx)  TO PRINTER  NOCONSOLE NODIALOG FOR &bFiltro.
		ENDIF
...
* Esperamos hasta que la impresora esté lista o que 10 segundos hayan pasado
*waitcenter("Por Favor Espere ...",.t.)
		c = 0
...
PROCEDURE proclase
*--------------------------------------------
* Definición de la clase que maneja los eventos:
DEFINE CLASS PDFEvent AS CUSTOM

	IMPLEMENTS __clsPDFCreator IN "PDFCreator.clsPDFCreatorOptions"
* Acontecimiento que indica si la impresora está lista
...
*
* Descripción.: Improme un reporte en formato PDF utilizando FOXYPREVIEWER.APP
*
* Parámetros..: lcnomfrx - Ruta y nombre del reporte
*               lcarchivopdf  - Nombre del archivi PDF a generar
*               lcDirPDF    - Ruta del archivi PDF
...
TRY
	...
	cfilePDF  = ADDBS(lcDirPDF)+FORCEEXT(lcarchivopdf, "PDF")

	IF EMPTY(bFiltro)
		REPORT FORM (lcnomfrx)  TO FILE (cfilePDF) OBJECT TYPE 10&&ARCHIVO CON EXTENSION PD
	ELSE
		REPORT FORM (lcnomfrx) FOR &bFiltro. TO FILE (cfilePDF) OBJECT TYPE 10&&ARCHIVO CON EXTENSION PDF
	ENDIF
...
*---------------------------------------------------------------------------*
*                                                                           *
* Show a centered visual foxpro wait window.                                *
*                                                                           *
*---------------------------------------------------------------------------*
* PARAMETERS:                                                               *
* ===========                                                               *
*  tcMessaje: Message text to show in the wait window.                      *
*  tlNoClear: If is true, the wait window run with NOCLEAR option.          *
*---------------------------------------------------------------------------*
*   Created: 01/03/2003                                                     *
* Last.Upd.: 02/06/2004                                                     *
*---------------------------------------------------------------------------*
...
*----------------------------------------------------------------------------------------
FUNCTION waitcenter
LPARAMETERS tcMessaje AS STRING, tlNoClear AS Boolean
LOCAL lcMsgText AS STRING

lcMsgText = ""
IF LEN( tcMessaje ) > 40
	FOR niCount = 1 TO LEN( tcMessaje ) STEP 40
		IF NOT EMPTY( lcMsgText )
			lcMsgText = lcMsgText + CHR( 13 )
		ENDIF
		lcMsgText = lcMsgText + SUBSTR( tcMessaje, niCount, 40 )
	NEXT
ELSE
	lcMsgText = tcMessaje
ENDIF
...
*-		Default values
lnRow = ROUND( INT( WROWS() / 2 ), 0 )
lnCol = INT( WCOLS() / 2 )

lnCol = lnCol - ( FLOOR( LEN( lcMsgText ) / 2 ) )
IF tlNoClear
	WAIT WINDOW lcMsgText AT lnRow, lnCol NOCLEAR NOWAIT
ELSE
	WAIT WINDOW lcMsgText AT lnRow, lnCol NOWAIT
ENDIF

ENDFUNC

*-------------------------------------------------------
* Función.....: ExisteCampo(cCampo, cAlias)
*
* Descripción.: Aumenta en la unidad pasada como parámetro
*               la fecha indicada.
*
* Parámetros..: cCampo	- Campo a buscar
*               cAlias  - Tabla donde buscar
*
*
* Devuelve....: Nùmerico
*				0 - No existe el campo
* 				1 - Existe el campo
*       		-1 - Error
*-------------------------------------------------------
FUNCTION ExisteCampo(cCampo, cAlias)
LOCAL cValorRet, lcAlias, lcCampo, gnFieldcount,  nCampo, lcErr, loError
...
TRY
	...
	IF PARAMETERS() < 2 OR TYPE("cAlias") <> "C"
		lcAlias = ALIAS()
	ELSE
		lcAlias = cAlias
	ENDIF
	...
	IF TYPE("cCampo") <> "C"
		MESSAGEBOX("Parámetros inválidos",0+16,"Error")
		cValorRet = -1
		EXIT
	ENDIF

	IF !USED(lcAlias)
		MESSAGEBOX("Alias "+ lcAlias + " no existe",0+16,"Error")
		cValorRet = -1
		EXIT
	ENDIF
	...
	lcCampo = UPPER(ALLTRIM(cCampo))
	gnFieldcount = AFIELDS(gaMyArray,lcAlias)
	IF gnFieldcount > 0

		nCampo = ASCAN(gaMyArray, lcCampo )
		IF nCampo > 0
			cValorRet =  1
		ELSE
			cValorRet =  0
